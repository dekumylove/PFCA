...
class StageAware(nn.Module):
    """
    StageAware model for patient disease prediction using EHR data.
    Implements a hierarchical attention mechanism over patient features and their relations.
    """
    def __init__(self, input_dim, output_dim, hidden_dim, num_rel=11, num_feat=1992, 
                 num_visit=6, max_feat=12, max_target=8, Lambda=0.1, 
                 bidirectional=True, device=torch.device('cuda')):
        """
        Args:
            ...
            num_feat (int): Number of possible features
            num_visit (int): Maximum number of visits
            max_feat (int): Maximum features per visit
            max_target (int): Maximum target predictions
            Lambda (float): Feature combination weight
        ...
        self.w_r = nn.Parameter(torch.rand(2 * hidden_dim))  # Relation-level attention
    ...
    def forward(self, features, feat_index, neighbor_index, rel_index, h_time):
        ...
        # Calculate relation-level attention
        ...
        a_r_i = a_r_i.unsqueeze(dim=1).unsqueeze(dim=1).unsqueeze(dim=1).repeat(
            1, self.num_visit, self.max_feat, self.max_target, 1)
        a_r_i = torch.einsum('bvftr, bvftr -> bvftr', rel_index, a_r_i)

        # Calculate node-level attention
