...
def select_top_diagnoses():
    ...
    df_top_diagnoses.to_csv('../data/mimic-iii/top_diagnoses_iii.csv', index=False)

# filter samples based on the 90 most common diagnoses
def filter_visits(ratio = 0.7):
    filtered_visit = []
    patient_diagnoses = pd.read_csv('../data/mimic-iii/diagnoses_icd.csv')
    top_diagnoses = pd.read_csv('../data/mimic-iii/top_diagnoses_iii.csv')
    ...
    filtered_visits_df.to_csv(f'../data/mimic-iii/filtered_visits_{ratio}_iii.csv')

# generate samples according to the visits filtered
def generate_sample():
    diagnose_data = pd.read_csv('../data/mimic-iii/admissions.csv')
    filtered_visits = pd.read_csv('../data/mimic-iii/filtered_visits_0.7_iii.csv')
    ...
    sample_df.to_csv('../data/mimic-iii/sample_0.7_iii.csv', index = False)
    print("only_one_num: ", only_one_num)
    print('sample_num:',sample_num)

# generate code_map based on the sample
def get_code_map():
    diagnose_data = pd.read_csv('../data/mimic-iii/diagnoses_icd.csv')
    procedure_data = pd.read_csv('../data/mimic-iii/procedures_icd.csv')
    prescription_data = pd.read_csv('../data/mimic-iii/prescriptions.csv')
    samples = pd.read_csv('../data/mimic-iii/sample_0.7_iii.csv')
    print(len(samples))
    ...
    for k, v in drug_code_map.items():
        if k not in code_map:
            ...
            index += 1
    print(len(code_map))
    with open('../data/mimic-iii/code_map_10.pkl', 'wb') as f:
...
def icd_to_text():
    ...
    pro_to_text = {}
    drug_to_text = {}
    with open('../data/mimic-iii/dia_code_map_10.pkl', 'rb') as f:
        ...
        with open('../data/mimic-iii/dia_to_text.pkl', 'wb') as e:
            pickle.dump(dia_to_text, e)

    with open('../data/mimic-iii/pro_code_map_10.pkl', 'rb') as f:
        ...
        with open('../data/mimic-iii/pro_to_text.pkl', 'wb') as e:
            pickle.dump(pro_to_text, e)
    with open('../data/mimic-iii/pro_to_text.pkl', 'rb') as f:
        pro_to_text = pickle.load(f)
        with open('../data/mimic-iii/pro_to_text.pkl', 'wb') as e:
            pickle.dump(pro_to_text, e)

    with open('../data/mimic-iii/drug_code_map_10.pkl', 'rb') as f:
        ...
        with open('../data/mimic-iii/drug_to_text.pkl', 'wb') as e:
            pickle.dump(drug_to_text, e)

# convert icd_text to CUI
def text_to_cui():
    dia_to_cui = {}
    with open('../data/mimic-iii/dia_to_text.pkl', 'rb') as f:
    ...
    with open('../data/mimic-iii/dia_cui.pkl', 'wb') as f:
    ...
    with open('../data/mimic-iii/pro_cui.pkl', 'wb') as f:
        pickle.dump(pro_to_cui, f)

    drug_to_cui = {}
    with open('../data/mimic-iii/drug_to_text.pkl', 'rb') as f:
        ...
        for key, values in tqdm(drug_to_text.items(), total = len(drug_to_text), desc = "drug_to_text"):
            ...
            if match and matches:
                ...
                print(f"no matching result for {key}")
    with open('../data/mimic-iii/drug_cui.pkl', 'wb') as f:
...
def extract_relations():
    ...
    with open('../data/mimic-iii/relations.pkl', 'wb') as f:
...
def extract_triplets():
    ...
    with open('../data/mimic-iii/relations.pkl', 'rb') as f:
        ...
        with open('../data/mimic-iii/icd_triples.pkl', 'wb') as f:
